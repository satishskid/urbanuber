...
// .wrangler/tmp/bundle-AyPt6G/checked-fetch.js
...
// .wrangler/tmp/bundle-AyPt6G/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-AyPt6G/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-AyPt6G/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function generateLiveKitToken(apiKey, apiSecret, roomName, participantIdentity, participantName, ttlSeconds = 3600) {
...
__name(generateLiveKitToken, "generateLiveKitToken");

// src/index.ts
var app = new Hono2();
app.use("/api/*", cors());
app.get("/health", (c2) => c2.json({ status: "ok" }));
app.post("/api/scribe", async (c2) => {
  try {
    const { audioTranscript, patientContext } = await c2.req.json();
    if (!audioTranscript) {
      return c2.json({ error: "audioTranscript is required" }, 400);
    }
    const groqApiKey = c2.env.GROQ_API_KEY;
    const aiGatewayUrl = c2.env.AI_GATEWAY_URL;
...
app.get("/api/upload-url", async (c2) => {
  try {
    const fileName = c2.req.query("filename");
    const contentType = c2.req.query("contentType");
    if (!fileName || !contentType) {
      return c2.json({ error: "filename and contentType query parameters are required" }, 400);
    }
    const accountId = c2.env.CF_ACCOUNT_ID;
    const accessKeyId = c2.env.R2_ACCESS_KEY_ID;
    const secretAccessKey = c2.env.R2_SECRET_ACCESS_KEY;
    if (!accountId || !accessKeyId || !secretAccessKey) {
      return c2.json({ error: "R2 credentials not fully configured in environment" }, 500);
    }
    ...
    return c2.json({ url, fileKey });
  } catch (err) {
    console.error("Upload URL Error:", err);
    return c2.json({ error: "Failed to generate presigned URL" }, 500);
  }
});
app.post("/api/video/token", async (c2) => {
  try {
    const { roomName, identity, name } = await c2.req.json();
    if (!roomName || !identity) {
      return c2.json({ error: "roomName and identity are required" }, 400);
    }
    const apiKey = c2.env.LIVEKIT_API_KEY;
    const apiSecret = c2.env.LIVEKIT_API_SECRET;
    const livekitUrl = c2.env.LIVEKIT_URL;
    if (!apiKey || !apiSecret) {
      return c2.json({ error: "LiveKit credentials not configured" }, 500);
    }
    ...
    return c2.json({ error: "Failed to generate video token" }, 500);
  }
});
var src_default = app;
