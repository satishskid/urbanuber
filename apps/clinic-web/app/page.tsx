...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const subs: PollSubscription<any>[] = [];
    ...
    subs.push(
      pollData(
        ...
        (providers) =>
          setStats((prev) =>
            prev.map((s) =>
              s.label === "Online Providers"
                ? { ...s, value: `${providers.length}` }
                : s,
            ),
          ),
        5000,
    ...
    subs.push(
      pollData(
        () => getServiceRequestsByStatus("pending"),
        (requests) =>
          setStats((prev) =>
            prev.map((s) =>
              s.label === "Pending Requests"
                ? { ...s, value: String(requests.length) }
                : s,
            ),
          ),
        5000,
    ...
    // Ledger events (activity feed)
    subs.push(
      pollData(
        () => getClinicalLedger(10),
        (entries) => {
          setStats((prev) =>
            prev.map((s) =>
              s.label === "Ledger Events"
                ? { ...s, value: String(entries.length) }
                : s,
            ),
          );
          const items: ActivityItem[] = entries.map((e: any) => {
            ...
            if (eventType === "TRIAGE")
              text = `Triage submitted by Provider ${providerId}`;
            if (eventType === "RX_SIGNED")
              text = `Prescription signed by Provider ${providerId}`;
            if (eventType === "LAB_UPLOAD")
              text = `Lab result uploaded for request ${e.request_id || "?"}`;
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Real-time overview of Skids healthcare operations
        </p>
      </div>
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="glass-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span
                className={`text-2xl w-10 h-10 flex items-center justify-center rounded-xl ${stat.color}`}
              >
                {stat.icon}
              </span>
            </div>
        ...
        <div className="lg:col-span-2 glass-card p-6">
          ...
          {activity.length === 0 ? (
            <p className="text-sm text-gray-600 py-8 text-center">
              No clinical ledger events yet. Events will appear here in
              real-time as providers submit consults.
            </p>
          ) : (
